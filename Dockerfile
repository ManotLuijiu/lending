...
# ─────────────────────────────────────────────────────────────────────
# Lending custom image for Frappe Docker
#
...
#
#   export CUSTOM_IMAGE=bunchee/lending
...
ARG APP_REPO=https://github.com/ManotLuijiu/lending
...
RUN apk add --no-cache git \
 ...
 && cd /tmp/app \
 && (git log -1 --format='lending @ %H | %s | %an <%ae>' > /tmp/app_version.txt) \
...
LABEL org.opencontainers.image.title="Lending" \
      org.opencontainers.image.description="Lending extension" \
      org.opencontainers.image.source="https://github.com/ManotLuijiu/lending" \
...
COPY --chown=frappe:frappe --from=fetcher /tmp/app /home/frappe/frappe-bench/apps/lending
COPY --chown=frappe:frappe --from=fetcher /tmp/app_version.txt /home/frappe/frappe-bench/apps/lending_version.txt
...
RUN bench setup requirements \
 ...
 && (python -c "import lending; print('Lending:', getattr(lending, '__version__', 'unknown'))" || echo "  (skip per-app import — module name may differ)") \
 && cat /home/frappe/frappe-bench/apps/lending_version.txt \
 && echo "✅ Lending image built from ${APP_REF}"
